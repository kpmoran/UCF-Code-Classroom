    ...
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <CardDescription>
              A “Feedback” pull request pinned to the assignment’s starting state, so its diff
              shows the student’s whole submission and you can comment line by line.
            </CardDescription>
          </div>
        ...
        {enabled ? (
          <>
            <div className="grid gap-3 sm:grid-cols-3 text-sm">
              <Stat label="With a pull request" value={withPr} tone="success" />
              <Stat
                label="Pushed, awaiting one"
                value={pushedWithoutPr}
                tone={pushedWithoutPr > 0 ? 'warning' : 'neutral'}
              />
              <Stat label="Not pushed yet" value={awaitingFirstPush} tone="neutral" />
            </div>

            <p className="text-xs text-muted">
              A pull request can only be opened once a student has pushed — GitHub refuses one
              with no changes to show. Repositories in the last column are waiting for that, not
              broken.
            </p>
          </>
        ) : (
          <p className="text-sm text-muted">
            Turning this on records a baseline in each repository and opens a pull request for
            every student who has already pushed.
          </p>
        )}
      </CardContent>
      ...
      <CardFooter className="flex justify-between gap-2">
        <Button
          ...
          size="sm"
          disabled={pending}
          onClick={() => run(setFeedbackPrEnabled, { enabled: enabled ? 'false' : 'true' })}
        >
